...
def Netual_process(alpha_data,industry):
...
def IC_computing(path,industry):
    ...
    return_data = pd.read_pickle(r'G:\short_period_mf\dailyreturn.pickle')
    return_data = return_data.rename(columns={'symbol':'code'})
    new_data = pd.merge(data,return_data,on = ['code'])
    dailyReturn = new_data.daily_return
    factors = Netual_process(data,industry)
    IC = dailyReturn.corr(factors)
...
for saf in standard_alpha:
    ...
    Netual_process(alpha_d,industry3)
#
